...
def process_output(output, stamp):
    for i in range(3):
        for j in range(3):
            output[i*4,j*4] = stamp[i,j]
            output[i*4 + 1,j*4 + 1] = stamp[i,j]
            output[i*4 + 2,j*4 + 2] = stamp[i,j]

            output[i*4,j*4 + 1] = stamp[i,j]
            output[i*4,j*4 + 2] = stamp[i,j]

            output[i*4 + 1,j*4] = stamp[i,j]
            output[i*4 + 2,j*4] = stamp[i,j]

            output[i*4 + 2,j*4 + 1] = stamp[i,j]
            output[i*4 + 2,j*4 + 1] = stamp[i,j]

            output[i*4 + 1,j*4 + 2] = stamp[i,j]
            output[i*4 + 1,j*4 + 2] = stamp[i,j]
    return output
