...
def generate_config():
    config = {
        'mid_color': random.choice(list(range(0,9)))
    }
    return config
...
def generate_example(config):
    ...
    while mid_color in line_colors:
        line_colors = random.sample(range(0, 9), 1 + nb_lines*2)
    ...
    for i in range(1, nb_lines*2, 2):
        ...
        # meet in the middle with mid_color dot
        output[y, half_width] = mid_color
        output[y, :half_width] = line_colors[i]
        output[y, half_width+1:] = line_colors[i+1]
